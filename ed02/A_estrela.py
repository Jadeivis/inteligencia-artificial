...
def heuristica_manhattan(estado):
    ...
    for i, valor in enumerate(estado):
        ...
        linha_objetivo, coluna_objetivo = divmod(valor - 1, 3)
        distancia += abs(linha_atual - linha_objetivo) + abs(coluna_atual - coluna_objetivo)
    return distancia

def busca_a_estrela(estado_inicial):
    fila_prioridade = []
    heapq.heappush(fila_prioridade, (heuristica_manhattan(estado_inicial), 0, estado_inicial, []))
    visitados = set()

    while fila_prioridade:
        custo_total, custo_g, estado_atual, caminho = heapq.heappop(fila_prioridade)
        ...
        for direcao in MOVIMENTOS_POSSIVEIS:
            ...
            if novo_estado and novo_estado not in visitados:
                ...
                heuristica = heuristica_manhattan(novo_estado)
                heapq.heappush(fila_prioridade, (novo_custo_g + heuristica, novo_custo_g, novo_estado, novo_caminho))
...
def resolver_quebra_cabeca_com_metricas(estado_inicial):
    ...
    caminho_solucao = busca_a_estrela(estado_inicial)
